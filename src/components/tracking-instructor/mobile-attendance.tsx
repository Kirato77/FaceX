...
export function MobileAttendance(props: MobileAttendanceProps) {
	...
	const connectWebSocket = async () => {
		try {
			...
			const socket = new WebSocket(`wss://localhost:8765?token=${token}`);
			setWs(socket);
			...
			socket.onopen = () => {
				...
				showToast({
					...
					description: "Connexion au serveur WebSocket sécurisée réussie",
				});
			};
			...
			socket.onclose = () => {
				...
				showToast({
					variant: "destructive",
					title: "Déconnexion",
					description: "Connexion au serveur WebSocket perdue",
				});
			};
			...
			socket.onerror = (error) => {
				showToast({
					...
					description:
						"Impossible de se connecter au serveur de manière sécurisée",
				});
			};
