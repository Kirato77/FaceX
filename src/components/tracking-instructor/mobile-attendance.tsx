		...
		try {
			...
			if (!userContext.user() || userContext.user()?.role !== "instructor") {
				showToast({
					...
					title: "Accès refusé",
					description:
						"Seuls les professeurs peuvent utiliser cette fonctionnalité",
				});
				return;
			}
			...
			socket.onclose = (event) => {
				...
				if (event.code === 1015) {
					errorMessage =
						"Erreur de connexion au serveur. Vérifiez que le serveur est correctement configuré.";
				}
						...
						if (response.data) {
							if (response.data.total_students) {
								...
								showToast({
									...
									description: `${response.data.total_students} étudiants à scanner`,
								});
								...
								showToast({
									...
									description: `${response.data.attendance_count}/${sessionStats().totalStudents} étudiants présents`,
								});
								...
								showToast({
									...
									description: `Session terminée : ${response.data.attendance_count}/${sessionStats().totalStudents} présences en ${response.data.duration}`,
								});
							}
						}
					}
			...
			showToast({
				variant: "destructive",
				title: "Erreur",
				description: `Impossible d'établir une connexion: ${(error as Error).message}`,
			});
		}
	};
